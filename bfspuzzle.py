...
def build_bfs_steps(start_state, goal_state, max_expand=60):
    queue = deque()
    visited = {start_state}
    node_counter = 0
    steps = []
        ...
        steps.append(
            {
                "expanded": current,
                "children": children,
                "frontier": [item["name"] for item in queue],
...
class BFSPuzzleUI:
    def __init__(self, root):
        self.root = root
        self.root.title("BFS 8-Puzzle Visualizer")
        self.root.geometry("980x650")
        self.root.minsize(860, 560)

        self.steps = build_bfs_steps(start, goal, max_expand=60)
        self.index = 0
    ...
    def build_layout(self):
        ...
        title = ttk.Label(header, text="BFS 8-Puzzle Visualizer", style="Title.TLabel")
        ...
        ttk.Button(controls, text="Previous", command=self.previous_step).grid(row=0, column=0, padx=5)
        ttk.Button(controls, text="Next", command=self.next_step).grid(row=0, column=1, padx=5)
        self.auto_button = ttk.Button(controls, text="Auto Run", command=self.toggle_auto)
        self.auto_button.grid(row=0, column=2, padx=5)
        ttk.Button(controls, text="Reset", command=self.reset).grid(row=0, column=3, padx=5)
        ...
        self.visited_label = self.make_info_label(info, "Visited")
        ...
        notebook.add(log_tab, text="Log BFS")
    ...
    def show_step(self, index):
        ...
        self.node_label.config(text=f"{node['name']} | cost = {node['cost']}")
        self.action_label.config(text=self.describe_path(node))
        self.visited_label.config(text=f"{step['visited_count']} trạng thái")
        self.frontier_label.config(text=", ".join(step["frontier"]) if step["frontier"] else "(rỗng)")

        if step["is_goal"]:
            self.status_label.config(text=f"Tìm thấy GOAL tại node {node['name']}!")
        else:
    ...
    def update_children(self, step):
        ...
        if not step["children"]:
            ...
            if step["is_goal"]:
                message = "Node này chính là GOAL."
            self.child_text.insert("end", message)
        else:
            for child in step["children"]:
                self.child_text.insert(
                    "end",
                    f"{child['name']} = {child['action']} của {child['parent']} | cost = {child['cost']}\n",
                )
    ...
    def update_log(self):
        ...
        for i, step in enumerate(self.steps[: self.index + 1], start=1):
            ...
            frontier = ", ".join(step["frontier"]) or "rỗng"
            self.log_text.insert(
                "end",
                f"Bước {i}: mở rộng {node['name']} | sinh: {children} | frontier: {frontier}\n",
            )
