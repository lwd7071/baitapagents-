...
# =========================
# THỨ TỰ SINH NODE
# IDS dùng DFS giới hạn độ sâu, nên stack vẫn là LIFO.
# =========================
moves = {
    "Up": (-1, 0),
    "Down": (1, 0),
...
def make_node(name, state, parent, action, depth, limit, path_states):
    return {
        "name": name,
        "state": state,
        "parent": parent,
...
def build_ids_steps(start_state, goal_state, max_depth=10):
    steps = []
    node_counter = 0
            ...
            if not is_goal:
                if current["depth"] >= limit:
                    ...
                    for action_name in moves:
                        ...
                        if new_state is None or new_state in current["path_states"]:
                            continue
                        ...
                        child = make_node(
                            ...
                            current["depth"] + 1,
                            limit,
                            current["path_states"] | {new_state},
                        )
                        node_counter += 1
                        children.append(child)

                    # Đưa vào stack theo thứ tự ngược để khi pop ra vẫn xét theo thứ tự moves.
            ...
            steps.append(
                {
                    "expanded": current,
                    "children": children,
                    "stack": [item["name"] for item in reversed(stack)],
                    "iteration_visited": iteration_visited,
                    "is_goal": is_goal,
                    "cut_off": cut_off,
                    "limit": limit,
...
class IDSPuzzleUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Trực quan hóa IDS 8-Puzzle")
        self.root.geometry("1000x660")
        self.root.minsize(900, 580)

        self.steps = build_ids_steps(start, goal, max_depth=10)
        self.index = 0
    ...
    def build_layout(self):
        ...
        title = ttk.Label(header, text="Trực quan hóa IDS 8-Puzzle", style="Title.TLabel")
        ...
        self.limit_label = self.make_info_label(info, "Giới hạn độ sâu")
        self.node_label = self.make_info_label(info, "Node đang xét")
        self.depth_label = self.make_info_label(info, "Độ sâu node")
        self.path_label = self.make_info_label(info, "Đường đi")
        self.visited_label = self.make_info_label(info, "Đã xét vòng này")
        self.stack_label = self.make_info_label(info, "Stack")
        ...
        notebook.add(log_tab, text="Nhật ký IDS")
    ...
    def show_step(self, index):
        ...
        self.limit_label.config(text=str(step["limit"]))
        self.node_label.config(text=node["name"])
        self.depth_label.config(text=str(node["depth"]))
        self.path_label.config(text=self.describe_path(node))
        self.visited_label.config(text=f"{step['iteration_visited']} node")
        self.stack_label.config(text=", ".join(step["stack"][:24]) if step["stack"] else "(rỗng)")

        if len(step["stack"]) > 24:
            self.stack_label.config(text=f"{', '.join(step['stack'][:24])}, ...")

        if step["is_goal"]:
            self.status_label.config(text=f"Tìm thấy trạng thái đích tại node {node['name']}!")
        elif step["cut_off"]:
            self.status_label.config(text=f"Node {node['name']} bị cắt vì đạt giới hạn độ sâu.")
        elif step["iteration_done"]:
            self.status_label.config(text=f"Kết thúc vòng lặp giới hạn {step['limit']}.")
        else:
            self.status_label.config(text=f"Đang xét node {node['name']}")
    ...
    def update_children(self, step):
        ...
        if step["is_goal"]:
            self.child_text.insert("end", "Node này chính là trạng thái đích.")
        elif step["cut_off"]:
            self.child_text.insert("end", "Không sinh node con vì node đã đạt giới hạn độ sâu.")
        elif not step["children"]:
            self.child_text.insert("end", "Không sinh node mới.")
        else:
            self.child_text.insert(
                "end",
                "Các node con được sinh trong vòng DFS giới hạn độ sâu hiện tại.\n\n",
            )
            for child in step["children"]:
                self.child_text.insert(
                    "end",
                    f"{child['name']} = {child['action']} của {child['parent']} | độ sâu = {child['depth']}\n",
                )
    ...
    def update_log(self):
        ...
        for i, step in enumerate(self.steps[: self.index + 1], start=1):
            node = step["expanded"]
            if step["limit"] != previous_limit:
                self.log_text.insert("end", f"\n=== Giới hạn độ sâu = {step['limit']} ===\n")
                previous_limit = step["limit"]

            children = ", ".join(child["name"] for child in step["children"]) or "không có"
            stack = ", ".join(step["stack"][:16]) or "rỗng"
            if len(step["stack"]) > 16:
                stack += ", ..."
            ...
            if step["is_goal"]:
                ...
                note = " | cắt do chạm giới hạn"

            self.log_text.insert(
                "end",
                f"Bước {i}: xét {node['name']} ở độ sâu {node['depth']} | sinh: {children} | stack: {stack}{note}\n",
            )
